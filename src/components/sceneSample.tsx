...
// テキストコンポーネントをメモ化して再レンダリングを減らす
// const TextContent = memo(function TextContent({ position }: { position: [number, number, number] }) {
//   return (
//     <Text
//       position={position}
//       fontSize={0.3}
...
//   const meshRef = useRef<Mesh>(null)
  ...
  // 画像のマテリアルを事前に作成してメモ化（再レンダリング時の再作成を防止）
  const imageMaterial = useMemo(() => new MeshBasicMaterial({ 
    transparent: true,
  ...
  useEffect(() => {
    // 不要なアニメーションループを削除
    // アニメーションフレームの更新は実際に変更が必要な場合のみ行う
        ...
        <div className="w-full h-full relative sticky top-0 inset-0 z-0">
          <div className="absolute inset-0 z-10">
            <Canvas 
              ...
              frameloop="demand" // 必要な時だけレンダリング更新
            >
